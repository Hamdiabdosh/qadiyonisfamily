...
export type AudioGuideHandle = {
  start: (section: GuideAudioSection) => void;
};

type GuideTrack = { src: string; label: string };

type Props = {
  guides: Partial<Record<GuideAudioSection, GuideTrack>>;
};
...
export const AudioGuidePlayer = forwardRef<AudioGuideHandle, Props>(function AudioGuidePlayer(
  { guides },
  ref,
) {
  const { t } = useI18n();
  ...
  const [activeSection, setActiveSection] = useState<GuideAudioSection | null>(null);

  const activeTrack = activeSection ? guides[activeSection] : undefined;
  const src = activeTrack?.src ?? "";
  const label = activeTrack?.label ?? t("audioGuide");
  ...
  useImperativeHandle(ref, () => ({
    start: (section: GuideAudioSection) => {
      const track = guides[section];
      if (!track?.src) return;
  ...
  if (!Object.values(guides).some((g) => g?.src)) return null;
  ...
  return (
    <>
      ...
      />
      {visible && src ? (
        <div className="mb-4 rounded-xl border border-primary/20 bg-card/90 p-3 shadow-sm backdrop-blur-sm">
